...
static void run_on_gpu(
    ...
    // Host buffers (using cudaHostAlloc for fast pinned-memory transfers)
    uint64_t* h_counts256 = nullptr;
    uint64_t* h_start_scalars = nullptr;
    ck(cudaHostAlloc((void**)&h_counts256, threadsTotal * 4 * sizeof(uint64_t), cudaHostAllocDefault), "cudaHostAlloc h_counts256");
    ck(cudaHostAlloc((void**)&h_start_scalars, threadsTotal * 4 * sizeof(uint64_t), cudaHostAllocDefault), "cudaHostAlloc h_start_scalars");
    ...
    auto reinit_chunk = [&](const uint64_t chunk_start[4]) {
        ...
        cudaMemcpyAsync(d_start_scalars, h_start_scalars,
                   threadsTotal * 4 * sizeof(uint64_t), cudaMemcpyHostToDevice, streamKernel);
        cudaMemcpyAsync(d_counts256,     h_counts256,
                   threadsTotal * 4 * sizeof(uint64_t), cudaMemcpyHostToDevice, streamKernel);
        int bs = (int)((threadsTotal + threadsPerBlock - 1) / threadsPerBlock);
        scalarMulKernelBase<<<bs, threadsPerBlock, 0, streamKernel>>>(d_start_scalars, d_Px, d_Py, (int)threadsTotal);
    };
